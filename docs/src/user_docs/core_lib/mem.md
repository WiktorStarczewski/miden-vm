...
| `pipe_double_words_to_memory`          | Copies an even number of words from the advice stack to memory while updating an Eidos state.<br/><br/>**Inputs:** `[R0, R1, C, write_ptr, end_ptr]`<br/>**Outputs:** `[R0', R1', C', write_ptr]`<br/><br/>Where `R0`, `R1`, and `C` are the two block words and BlakeG chaining value (`R0` on top).<br/><br/>Notice that the `end_ptr - write_ptr` value must be positive and a multiple of 8. |
| `pipe_words_to_memory`                 | Copies an arbitrary number of words from the advice stack to memory while updating an Eidos state.<br/><br/>**Inputs:** `[num_words, write_ptr]`<br/>**Outputs:** `[R0, R1, C, write_ptr']`<br/><br/>Where `R0`, `R1`, and `C` are the final VM hasher state (`R0` on top). |
