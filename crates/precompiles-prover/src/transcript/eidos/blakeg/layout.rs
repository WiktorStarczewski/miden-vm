//! Column layout for the PVM's 32-row x 128-column BlakeG compression core.
...
/// buses to the physical compression which consumes them. Footer column 125 overlays a fused-row
/// carry column; it is not an additional committed column.
...
const _: () = assert!(F_COMPRESSION_CYCLE_ID_COL < NUM_COLS);
